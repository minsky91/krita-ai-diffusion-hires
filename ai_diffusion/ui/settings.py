...
# minsky91
class HiresSettings(SettingsTab):
    def __init__(self):
        # most hires settings require special custom nodes, a managed Comfy server is a no-go
        if settings.server_mode is ServerMode.external:
            super().__init__(_("High Resolution Settings"))
        else:
            super().__init__(_("Extras"))
        ...
        self._widgets["save_gen_data"].reset_files(gen_info_choices)

        self.add("metadata_verbosity", ComboBoxSetting(S._metadata_verbosity, parent=self))
        metadata_verbosity_settings = [(_("Essential"), 0), (_("Medium"), 1), (_("Verbose"), 2)]
        self._widgets["metadata_verbosity"].set_items(metadata_verbosity_settings)

        self.add("logfile_verbosity", ComboBoxSetting(S._logfile_verbosity, parent=self))
        logfile_verbosity_settings = [(_("Essential"), 0), (_("Medium"), 1), (_("Verbose"), 2)]
        self._widgets["logfile_verbosity"].set_items(logfile_verbosity_settings)
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, server: Server):
        ...
        # minsky91: hires additions
        self.hires = HiresSettings()
        ...
        # minsky91
        create_list_item(_("Hires"), self.hires)
