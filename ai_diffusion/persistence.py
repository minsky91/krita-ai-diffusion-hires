...
# minsky91
from .image import Image, ImageFileFormat
...
def import_prompt_from_file(model: Model):
    ...
    if model.regions.positive == "" and model.regions.negative == "" and filename.endswith(exts):
        try:
            ...
            # A1111 (minsky91: or Krita AI with metadata supported)
            if text := reader.text("parameters"):
                ...
                # minsky91: recover & restore style from a Krita AI-written png file 
                style_filename = JobParams.recover_style_preset(text)
                if style_filename != "":
                    model.style = Styles.list().find(style_filename) or Styles.list().default
                    if model.style.name != Styles.list().default.name:
                        if Verbosity(settings.logfile_verbosity) in [Verbosity.verbose]: 
                            log.info(f"import_prompt_from_file: found {model.style.name} style as {style_filename} in {filename}, applying to the document")
                        elif Verbosity(settings.logfile_verbosity) in [Verbosity.medium]: 
                            log.info(f"found {model.style.name} style in the input imnge, applying to the document")
                # end of minsky91 additions
            # ComfyUI
