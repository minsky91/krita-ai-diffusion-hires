...
# produce a unique CRC32-based id of an image (includes a simplified version of Image.to_array())
def image_uid(image: Image):
    #return id(image)    # the simplest one, but not good enough for the purpose
    image.to_numpy_format()
    w, h = image.extent
    c = 4 if image.is_rgba else 1
    bits = image._qimage.constBits()
    return crc32(bits.asarray(w * h * c))    
    #return id(image)
...
class ComfyWorkflow:
    ...
    # minsky91: modified name to avoid clashes with TiledDiffusion logic
    def vae_decode_tiled_512(self, vae: Output, latent_image: Output):
        return self.add(
    ...
    def load_image(self, image: Image | ImageCollection):
        if self._run_mode is ComfyRunMode.runtime:
            ...
            if settings.upload_method != "None":
                return self._load_image_batch(image, self._load_image_transient, self.batch_image)
            # end of minsky91 additions
    ...
    def load_mask(self, mask: Image | ImageCollection):
        if self._run_mode is ComfyRunMode.runtime:
            ...
            # minsky91: support for loading transient mask images stored in binary or text-encoded format
            if settings.upload_method != "None":
                return self._load_image_batch(mask, self._load_mask_transient, self.batch_mask)
            # end of minsky91 additions
    ...
    # minsky91 (m91) / version with adaptable output format 

    def send_image_http(self, image: Output, format: str, uid: str = ""):
        return self.add("ETN_SendImagesTransient", 1, images=image, format=format, uid=uid)

    def save_image_transient(self, image: Output, prefix: str):
...
# minsky91 / added experimental support for TiledDiffusion
    ...
    def tiled_diffusion(
        self,
        model: Output,
        method: str = "Mixture of Diffusers",
        tile_width: int = 1024,
    ...
    def vae_decode_tiled(self, vae: Output, latent_image: Output, tile_size: int = 1024, overlap: int = 64, temporal_size: int = 64, temporal_overlap: int = 8):
