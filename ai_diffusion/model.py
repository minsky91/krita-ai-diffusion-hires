...
class Model(QObject, ObservableProperties):
    ...
    def _prepare_workflow(self, dryrun=False):
        ...
        # minsky91: implemented collecting full metadata
        self.set_job_metadata(job_params, input, prompt, job_regions)
        # end of minsky91 additions
        
        return input, job_params

    # minsky91: extended the collected metadata code
    def set_job_metadata(self, job_params, input, prompt, job_regions):
        ...
        if input.images.hires_mask is not None:
            job_params.metadata["mask_resolution"] = [input.images.hires_mask.width, input.images.hires_mask.height] 
        canvas_w, canvas_h = self.document.extent
        control_meta: list[str] = [] 
        control_uid_modes: dict[int, str] = {}
        if input.conditioning is not None:
            control_ix = 0
            for control in input.conditioning.control:
                control_ix += 1
                if control.image:
                    control_uid = image_uid(control.image)
                    if control_uid in control_uid_modes and control_uid_modes[control_uid] == control.mode.text:
                        continue   # skip duplicate control layers
                    control_uid_modes[control_uid] = control.mode.text
                    if control.image.width != canvas_w or control.image.height != canvas_h:
                        res_str = f", resolution {control.image.width}x{control.image.height}"
                    else:
                        res_str = ""
                else:
                    res_str = ""
                param_str = f"strength {control.strength:.2f}, range {control.range[0]:.2f}-{control.range[1]:.2f}"
                control_str = f"Control layer {control_ix} ({control.mode.text}): {param_str}{res_str}"
                control_meta.append(control_str) 
        job_params.metadata["control_layers"] = control_meta
        region_meta: list[str] = [] 
        if job_regions is not None:
            region_ix = 0
            for region in job_regions:
                region_ix += 1
                if region.bounds.width != canvas_w or region.bounds.height != canvas_h:
                    res_str = f", resolution {region.bounds.width}x{region.bounds.height}"
                else:
                    res_str = ""
                region_str = f"Region {region_ix}: prompt <{region.prompt}>{res_str}"
                region_meta.append(region_str) 
        job_params.metadata["regions"] = region_meta
        ...
        if self.workspace is not Workspace.live:
            ...
            exclude = [  # exclude control layers from projection
                c.layer for c in self.regions.control if not c.mode.is_part_of_image 
                    and not active_id == c.layer_id  # minsky91: this should be properly resolved elsewhere
            ]
    ...
    def handle_message(self, message: ClientMessage):
        ...
        # end of minsky91 additions
            ...
            if isinstance(message.workflow, str):
                self.jobs.set_workflow(job, message.workflow)
            if isinstance(message.metadata, list):
                self.jobs.set_stat_metadata(job, message.metadata)
            # end of minsky91 additions
...
# end of minsky91 additions
    ...
    def apply_result(
        ...
        # minsky91: had to disable the apply region-related logic, as it places absolutely useless gened region layers
        # atop of the source ones, and grouping them together only makes this worse. Basically it looks like a mess
        #if True:
...
def _save_job_result(model: Model, job: Job | None, index: int):
    ...
    # minsky91: setting-based selection of the output format png/jpeg
    format_ext = ".png"
    if max(result_image.width, result_image.height) >= settings.jpeg_res_threshold * 1024:
        format_ext = ".jpg"
    # end of minsky91 additions

    path = path.parent / (base_filename+format_ext)
    ...
    if "As a text file" in settings.save_gen_data:
        ...
        if Verbosity(settings.logfile_verbosity) in [Verbosity.verbose]: 
            log.info(f"_save_job_result: saving gen stats metadata in {txt_filepath}, settings.save_gen_data {settings.save_gen_data}")
        with open(txt_filepath, "w") as f:
            for meta_line in metadata:
                f.write(meta_line+"\n")

    # embed gen data & workflow in the output file (works only with png alas, kind of,
    #  as comfyui frontend ignores zTXt-compressed png chunks that QImage produces in setText()
    out_filepath = path.absolute().as_posix()  
    if "Workflow embedded in the png file" in settings.save_gen_data:
        the_workflow = job.get_workflow(job)
        if the_workflow is None:
            log.warning(f"_save_job_result: no workflow found in the gen results, ignored")
        elif len(the_workflow) > 100*1024 or len(the_workflow) <= 32:
            log.warning(f"_save_job_result: gen workflow too large or too small to be saved in the png file, ignored")
        else:
        #    with open(path.with_suffix(".json"), "w") as f:
        #        f.write(the_workflow)
            if Verbosity(settings.logfile_verbosity) in [Verbosity.verbose]: 
                log.info(f"_save_job_result: saving gen workflow of {len(the_workflow)/1024:.1f} KB size in {out_filepath}")
            base_image._qimage.setText("prompt", the_workflow)

    if "As metadata in the png file" in settings.save_gen_data:
